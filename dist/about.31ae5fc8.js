...
window.addEventListener('keydown', (event)=>{
    if (event.key === 'Escape' && modal.hasAttribute('open')) closeDialog();
});
...
window.addEventListener('popstate', (event)=>{
    ...
    if (event.state && event.state.page === 'my-email') openDialog();
    else if (modal.hasAttribute('open')) closeDialog();
});
// Verifica se a URL contém o hash '#my-email' quando a página é carregada
document.addEventListener('DOMContentLoaded', (event)=>{
    if (window.location.hash === '#my-email') openDialog();
});
/////////////////////////////////////////////////////////////////
...
async function copyToClipboard() {
    ...
    try {
        ...
        console.error("Erro ao copiar para a \xe1rea de transfer\xeancia:", error);
...
if (workDurationElement) workDurationElement.innerText = calculateWorkDuration(startDate);
/////////////////////////////////////////////////////////////////
...
document.addEventListener('click', function(event) {
    ...
    while(target != null && !target.matches('summary') && !target.matches('details'))target = target.parentElement;
    // Check if target is a summary child, find the summary in that case
    if (target && !target.matches('summary') && target.parentElement.matches('summary')) target = target.parentElement;
    ...
    if (target && target.matches('summary') && target.parentElement.matches('details')) {
        ...
        const onAnimationEnd = (callback)=>{
            contentElement.addEventListener('animationend', callback, {
                once: true
            });
        };
...
const onScroll = ()=>{
    ...
    if (currentScrollY === 0) // User has scrolled to the top
...
const observer = new MutationObserver((mutationsList)=>{
    for (let mutation of mutationsList)if (mutation.type === 'attributes' && mutation.attributeName === 'data-visible') {
        const visibility = mainNavList.getAttribute('data-visible');
        if (visibility === "true") isListeningToScroll = false;
        else if (visibility === "false") isListeningToScroll = true;
    }
});
// Start observing the target node for configured mutations
observer.observe(mainNavList, {
    attributes: true
});
/////////////////////////////////////////////////////////////////
...
//# sourceMappingURL=about.31ae5fc8.js.map
